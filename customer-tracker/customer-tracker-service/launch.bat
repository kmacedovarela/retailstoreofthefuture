...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "customer-tracker-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name customer-tracker-service apps/customer-tracker-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app customer-tracker-service:1.0-SNAPSHOT
		call oc expose svc/customer-tracker-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd customer-tracker-service
		cd target
