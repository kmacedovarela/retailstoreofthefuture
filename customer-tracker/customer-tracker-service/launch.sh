...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "customer-tracker-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name customer-tracker-service apps/customer-tracker-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app customer-tracker-service:1.0-SNAPSHOT
    oc expose svc/customer-tracker-service
else

	echo "Launching the application locally..."
	pattern="customer-tracker-service"
	files=( $pattern )
	cd ${files[0]}
